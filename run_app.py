...
# User Processing Helper Functions
...
def deleteCategory(category_name):
    ...
    # If logged in user != item owner redirect them
    if creator.id != login_session['user_id']:
        flash ("You cannot delete this Category. This Category was created by %s" % creator.name)
        return redirect(url_for('showCatalog'))
    if request.method =='POST':
...
def editItem(category_name, item_name):
    ...
    # If logged in user != item owner redirect them
    if creator.id != login_session['user_id']:
        flash ("You cannot edit this post. This post was created by %s" % creator.name)
        return redirect(url_for('showCatalog'))
...
def deleteItem(category_name, item_name):
    ...
    # If logged in user != item owner redirect them
    if creator.id != login_session['user_id']:
        flash ("You cannot delete this post. This item was created by %s" % creator.name)
        return redirect(url_for('showCatalog'))
    if request.method =='POST':
        session.delete(itemToDelete)
        session.commit()
        flash('Post Successfully Deleted. '+itemToDelete.name)
        return redirect(url_for('showCategory',
...
#    return jsonify(items=[i.serialize for x in items])
#
